...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_suggestions') {
    // Get evaluated faculty members with average scores less than 4.0 in any evaluation category (HR-created only)
    $teachers_query = "SELECT DISTINCT
                      ...
                      JOIN main_evaluation_categories mec ON es.main_category_id = mec.id AND mec.created_by_role IN ('human_resource', 'hr_manager')
                      JOIN evaluation_sub_categories esc ON mec.id = esc.main_category_id AND esc.created_by_role IN ('human_resource', 'hr_manager')
                      ...
                      WHERE es.evaluatee_type = 'teacher'
                        ...
                        AND es.created_by_role IN ('human_resource', 'hr_manager')
                        AND er.rating_value IS NOT NULL
                        AND f.is_active = 1
                      GROUP BY f.id, esc.id
                      HAVING average_score < 4.0 AND total_ratings >= 3
                      ORDER BY e.last_name, e.first_name, average_score ASC";
    ...
    while ($teacher = mysqli_fetch_assoc($teachers_result)) {
        // Find available trainings for this sub-category (HR-created only)
        $trainings_query = "SELECT id, title, type, start_date
                           FROM trainings_seminars
                           WHERE sub_category_id = ?
                             AND status = 'published'
                             AND start_date > NOW()
                             AND created_by_role IN ('human_resource', 'hr_manager')
...
$suggestions_query = "SELECT
                     ...
                     WHERE f.is_active = 1
                     GROUP BY f.id
                     ORDER BY critical_count DESC, high_count DESC, total_suggestions DESC, e.last_name, e.first_name";
...
// Get evaluated faculty members with overall low scores (for display) - HR-created only
...
<div class="mb-6 sm:mb-8">
    <div class="flex items-center justify-between">
        ...
        <div class="flex space-x-2">
            ...
            <a href="training-programs.php" class="btn-secondary">
                ...
                <div class="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div class="accordion-content bg-white" id="content-<?php echo $accordion_id; ?>">
                        <div class="p-4 border-t border-gray-200">
                            ...
                            <?php
                            // Get individual category scores for this faculty member (HR-created only)
        ...
        <?php if (mysqli_num_rows($suggestions_result) > 0): ?>
            ...
            <div class="border border-gray-200 rounded-lg p-6">
                ...
                $teacher_suggestions_query = "SELECT ts.*, ts2.title as training_title, ts2.type as training_type, esc.name as category_name,
                                            ...
                                            JOIN trainings_seminars ts2 ON ts.training_id = ts2.id AND ts2.created_by_role IN ('human_resource', 'hr_manager')
                                            LEFT JOIN evaluation_sub_categories esc ON ts.evaluation_category_id = esc.id AND esc.created_by_role IN ('human_resource', 'hr_manager')
                                            LEFT JOIN main_evaluation_categories mec ON esc.main_category_id = mec.id AND mec.created_by_role IN ('human_resource', 'hr_manager')
                                            WHERE ts.user_id = ?
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center space-x-2 text-xs text-gray-600">
                                ...
                                <span class="px-1 py-0.5 bg-blue-100 text-blue-700 rounded text-xs">
                                    <?php echo htmlspecialchars($suggestion['evaluator_type'] ?? 'Unknown'); ?>
                                </span>
                            </div>
                        </div>
