...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // Insert training/seminar with created_by_role
        $insert_query = "INSERT INTO trainings_seminars (
            ...
            status, is_mandatory, certificate_provided, materials_provided, cost, created_by, created_by_role
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($conn, $insert_query);
        $user_role = $_SESSION['role'];
        mysqli_stmt_bind_param($stmt, "sssiiidssssssiiids",
            $title, $description, $type, $category_id, $main_category_id, $sub_category_id,
            $duration_hours, $max_participants, $venue, $start_date, $end_date, $registration_deadline,
            $status, $is_mandatory, $certificate_provided, $materials_provided, $cost, $_SESSION['user_id'], $user_role
        );
...
// Get training categories created by HR users
...
// Get main evaluation categories created by HR users
...
<div class="mb-6 sm:mb-8">
    <div class="flex items-center justify-between">
        ...
        <a href="training-programs.php" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition text-center">
...
<div class="bg-white rounded-lg shadow-md p-6">
    <form method="POST" class="space-y-6">
        ...
        <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
            <a href="training-programs.php" class="bg-gray-600 text-white px-6 py-2 rounded-md hover:bg-gray-700 transition-colors">
                Cancel
            </a>
...
document.getElementById('main_category_id').addEventListener('change', function() {
    ...
    if (mainCategoryId) {
        // Fetch sub-categories for the selected main category (HR-created only)
        fetch(`get_sub_categories_hr.php?main_category_id=${mainCategoryId}`)
