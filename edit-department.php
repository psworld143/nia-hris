...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $icon = sanitize_input($_POST['icon'] ?? 'fas fa-building');
    $color_theme = sanitize_input($_POST['color_theme'] ?? '#FF6B35');
    ...
    if (empty($department_name)) {
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            // Update department
            $update_query = "UPDATE departments SET name = ?, code = ?, description = ?, icon = ?, color_theme = ?, sort_order = ?, is_active = ? WHERE id = ?";
            $update_stmt = mysqli_prepare($conn, $update_query);
            mysqli_stmt_bind_param($update_stmt, "sssssiis", $department_name, $department_code, $description, $icon, $color_theme, $sort_order, $is_active, $department_id);
            ...
            if (mysqli_stmt_execute($update_stmt)) {
                ...
                $department['color_theme'] = $color_theme;
