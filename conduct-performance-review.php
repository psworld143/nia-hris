...
LEFT JOIN performance_review_criteria prc2 ON prc.id = prc2.category_id AND prc2.is_active = 1
...
$criteria_query = "SELECT * FROM performance_review_criteria WHERE is_active = 1 ORDER BY category_id, display_order";
...
// Get existing scores
$scores_query = "SELECT criteria_id, score, comments FROM performance_review_scores WHERE performance_review_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $delete_scores_query = "DELETE FROM performance_review_scores WHERE performance_review_id = ?";
    ...
    $insert_score_query = "INSERT INTO performance_review_scores (performance_review_id, criteria_id, score, comments) VALUES (?, ?, ?, ?)";
    ...
    $delete_goals_query = "DELETE FROM performance_review_goals WHERE performance_review_id = ?";
    ...
    $insert_goal_query = "INSERT INTO performance_review_goals (performance_review_id, goal_description, target_date, status, completion_date, completion_notes) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $insert_goal_query);
    
    foreach ($_POST['goals'] as $goal_data) {
        if (!empty($goal_data['title'])) {
            $description = sanitize_input(($goal_data['title'] ?? '') . ($goal_data['description'] ?? '' ? ': ' . $goal_data['description'] : ''));
            $target_date = !empty($goal_data['target_date']) ? $goal_data['target_date'] : null;
            $status = sanitize_input($goal_data['status'] ?? 'pending');
            $completion_date = !empty($goal_data['completion_date']) ? $goal_data['completion_date'] : null;
            $comments = sanitize_input($goal_data['comments'] ?? '');
            
            mysqli_stmt_bind_param($stmt, "isssss", 
                $review_id, $description, $target_date, $status, $completion_date, $comments);
            mysqli_stmt_execute($stmt);
        }
    }
    }
...
<div class="mb-6">
    ...
    <i class="fas fa-edit mr-2"></i>Conduct Performance Review
                </h2>
                <p class="opacity-90">Evaluate employee performance and set development goals</p>
            </div>
...
<form method="POST" class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200 mb-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900"><?php echo htmlspecialchars($category['category_name'] ?? ''); ?></h3>
    ...
    <?php 
    // Parse goal_description to extract title if it contains ':'
    $goal_desc = $goal['goal_description'] ?? '';
    $goal_title = '';
    $goal_detail = $goal_desc;
    if (strpos($goal_desc, ':') !== false) {
    $parts = explode(':', $goal_desc, 2);
    $goal_title = trim($parts[0]);
    $goal_detail = trim($parts[1] ?? '');
    } else {
    ...
    <input type="text" 
       ...
       value="<?php echo htmlspecialchars($goal_title); ?>"
       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
    ...
    <select name="goals[<?php echo $index; ?>][status]" 
        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
    <option value="pending" <?php echo ($goal['status'] ?? 'pending') === 'pending' ? 'selected' : ''; ?>>Pending</option>
    ...
    <option value="cancelled" <?php echo ($goal['status'] ?? '') === 'cancelled' ? 'selected' : ''; ?>>Cancelled</option>
    </select>
                </div>
    ...
    <input type="date" 
       name="goals[<?php echo $index; ?>][completion_date]" 
       value="<?php echo $goal['completion_date'] ?? ''; ?>"
       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
            </div>
      ...
      rows="2"
      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
      placeholder="Describe the goal in detail..."><?php echo htmlspecialchars($goal_detail); ?></textarea>
            </div>
      ...
      rows="2"
      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
      placeholder="Add notes about goal completion..."><?php echo htmlspecialchars($goal['completion_notes'] ?? ''); ?></textarea>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
...
function addGoal() {
    ...
    <label class="block text-sm font-medium text-gray-700 mb-2">Completion Date</label>
    <input type="date" 
       name="goals[${goalIndex}][completion_date]" 
       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
            </div>
      ...
      rows="2"
      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
      placeholder="Add notes about goal completion..."></textarea>
            </div>
        </div>
    `;
