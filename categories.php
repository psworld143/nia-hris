...
// Get main evaluation categories created by HR users for the dropdown
$main_categories_query = "SELECT id, name, evaluation_type FROM main_evaluation_categories WHERE status = 'active' AND created_by_role IN ('human_resource', 'hr_manager') ORDER BY name ASC";
            ...
            case 'add_category':
                ...
                if (empty($name)) {
                    ...
                    if (mysqli_stmt_execute($insert_stmt)) {
                        ...
                        $sub_category_query = "INSERT INTO evaluation_sub_categories (main_category_id, name, description, created_by, created_by_role) VALUES (?, ?, ?, ?, ?)";
                        $sub_category_stmt = mysqli_prepare($conn, $sub_category_query);
                        $user_role = $_SESSION['role'];
                        mysqli_stmt_bind_param($sub_category_stmt, "issis", $main_category_id, $name, $description, $_SESSION['user_id'], $user_role);
...
$categories_query = "SELECT ec.*,
                    ...
                    WHERE (esc.created_by_role IN ('human_resource', 'hr_manager') OR esc.created_by_role IS NULL)
                    GROUP BY ec.id
                ...
                <div class="border border-gray-200 rounded-lg p-4">
                    ...
                    <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                        <?php if ($category['sub_category_id']): ?>
                            <a href="questionnaires.php?sub_category_id=<?php echo safe_encrypt_id($category['sub_category_id']); ?>"
