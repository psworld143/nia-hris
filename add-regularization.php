...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($employee_id) || empty($staff_category_id) || empty($date_of_hire) || empty($probation_start_date)) {
        $error = 'All required fields must be filled';
    } else {
        // Check if faculty already has a regularization record
        $check_query = "SELECT id FROM employee_regularization WHERE employee_id = ? AND is_active = 1";
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            if (!$category_data) {
                ...
                if (!$status_data) {
                    ...
                    $insert_query = "INSERT INTO employee_regularization (
                    ...
                    mysqli_stmt_bind_param($insert_stmt, "iiisssss", 
                        $employee_id, $staff_category_id, $status_data['id'], $date_of_hire,
...
// Get faculty members who don't have regularization records
    $faculty_query = "SELECT f.id, e.first_name, e.last_name, e.email, f.position, e.department, e.employee_id as employee_id, fd.date_of_hire
                  FROM employees f
                  LEFT JOIN employee_details fd ON f.id = fd.employee_id
                  WHERE f.is_active = 1 
                  AND f.id NOT IN (SELECT employee_id FROM employee_regularization WHERE is_active = 1)
    ...
    <form method="POST" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="employee_id" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-seait-orange focus:ring-2 focus:ring-seait-orange/20 transition-all bg-white shadow-sm">
