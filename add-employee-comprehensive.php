...
try {
    ...
    // Employment Details
    $employment_type = sanitize_input($_POST['employment_type'] ?? 'Probationary');
    ...
    $overtime_rate = $_POST['overtime_rate'] ? (float)$_POST['overtime_rate'] : null;
    $night_differential_rate = $_POST['night_differential_rate'] ? (float)$_POST['night_differential_rate'] : 0.10;
    $hazard_pay = $_POST['hazard_pay'] ? (float)$_POST['hazard_pay'] : 0.00;
    ...
    $passport_expiry = sanitize_input($_POST['passport_expiry'] ?? '');
    ...
    $check_employee_id_query = "SELECT id FROM employees WHERE employee_id = ? 
                               UNION ALL 
                               SELECT id FROM employees WHERE qrcode COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci";
    ...
    try {
        ...
        $insert_employee_query = "INSERT INTO employees (employee_id, first_name, last_name, email, password, position, department, employee_type, hire_date, phone, address, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        ...
        $insert_details_query = "INSERT INTO employee_details (
            ...
            basic_salary, salary_grade, step_increment, allowances, overtime_rate, 
            ...
            blood_type, languages_spoken, skills_competencies, notes, profile_photo, created_by, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        ...
        mysqli_stmt_bind_param($insert_details_stmt, 'issssssssssssssidsiddddsssssssssssssssssssissssi',
    ...
    echo json_encode([
        'success' => false,
        'message' => 'An error occurred while adding the employee: ' . $e->getMessage()
    ]);
} finally {
    // Restore autocommit
