...
$required_tables = ['trainings_seminars', 'main_evaluation_categories', 'evaluation_sub_categories'];
$missing_tables = [];

foreach ($required_tables as $table) {
    $check_query = mysqli_query($conn, "SHOW TABLES LIKE '$table'");
    if (mysqli_num_rows($check_query) == 0) {
        $missing_tables[] = $table;
    }
}

// If tables are missing, show migration page
if (!empty($missing_tables)) {
    $page_title = 'Setup Training System';
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <h2 class="text-3xl font-bold text-gray-900 mb-4">Training System Tables Not Found</h2>
            ...
            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 text-left">
                ...
                <ul class="text-blue-800 space-y-1 ml-4">
                    ...
                    <li>✓ Training categories and sub-categories</li>
                    <li>✓ Registration and materials management</li>
                    <li>✓ All existing training data and records</li>
                    <li>✓ Evaluation categories for training assessment</li>
                </ul>
            </div>
            ...
            <div class="flex space-x-4 justify-center">
                <button onclick="testConnection()" id="testBtn" 
                        class="bg-blue-500 text-white px-6 py-3 rounded-lg hover:bg-blue-600 transform transition-all hover:scale-105 font-medium shadow-lg">
                    <i class="fas fa-plug mr-2"></i>Test Connection
                </button>
                <button onclick="migrateSystem()" id="migrateBtn" 
                    ...
                    <i class="fas fa-database mr-2"></i>Migrate Training System
                </button>
            </div>
            
            <div id="migrateResult" class="mt-6 hidden"></div>
        </div>
    </div>
    ...
    function testConnection() {
        ...
        fetch('test-training-migration.php', {
            method: 'POST',
            headers: {
    ...
    function migrateSystem() {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Migrating tables...';
        
        fetch('migrate-training-system.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
                ...
                result.innerHTML = `
                    <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg">
                        ...
                        <p class="text-sm mb-4">
                            Tables: ${data.tables_migrated} | 
                            Records: ${data.records_migrated} | 
                            Failed: ${data.tables_failed}
                        </p>
                        <button onclick="location.reload()" 
                                class="bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600 transition-colors">
                            <i class="fas fa-arrow-right mr-2"></i>Continue to Training Programs
                        </button>
                    </div>
                `;
            } else {
...
// Get available categories for filter (HR-created only)
$categories_query = "SELECT id, name FROM training_categories WHERE status = 'active' AND created_by_role IN ('human_resource', 'hr_manager') ORDER BY name";
$categories_result = mysqli_query($conn, $categories_query);

// Get available main categories for filter (HR-created only)
$main_categories_query = "SELECT id, name FROM main_evaluation_categories WHERE status = 'active' AND created_by_role IN ('human_resource', 'hr_manager') ORDER BY name";
...
// Filter by creator role - HR users should only see trainings created by HR users
$where_conditions[] = "ts.created_by_role IN ('human_resource', 'hr_manager')";
...
// Get trainings/seminars with filters (HR-created only)
$trainings_query = "SELECT ts.*,
                   ...
                   LEFT JOIN main_evaluation_categories mec ON ts.main_category_id = mec.id AND mec.created_by_role IN ('human_resource', 'hr_manager')
                   LEFT JOIN evaluation_sub_categories esc ON ts.sub_category_id = esc.id AND esc.created_by_role IN ('human_resource', 'hr_manager')
...
// Get training statistics (HR-created only)
$stats_query = "SELECT
                ...
                WHERE created_by_role IN ('human_resource', 'hr_manager')";

$stats_result = mysqli_query($conn, $stats_query);
$stats = mysqli_fetch_assoc($stats_result);

// Get upcoming trainings (HR-created only)
$upcoming_query = "SELECT ts.*, tc.name as category_name, COUNT(tr.id) as registered_count
                   FROM trainings_seminars ts
                   LEFT JOIN training_categories tc ON ts.category_id = tc.id
                   LEFT JOIN training_registrations tr ON ts.id = tr.training_id
                   WHERE ts.status = 'published' AND ts.start_date > NOW() AND ts.created_by_role IN ('human_resource', 'hr_manager')
                   GROUP BY ts.id
...
// Get recent suggestions (HR-created trainings only)
$suggestions_query = "SELECT tsg.*, ts.title as training_title, u.first_name, u.last_name, esc.name as category_name
                      ...
                      JOIN trainings_seminars ts ON tsg.training_id = ts.id AND ts.created_by_role IN ('human_resource', 'hr_manager')
            ...
            <?php if (mysqli_num_rows($upcoming_result) > 0): ?>
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
                    <div>
                        ...
                        <p class="text-xs text-gray-600"><?php echo date('M d, Y', strtotime($training['start_date'])); ?> • <?php echo htmlspecialchars($training['category_name'] ?? 'N/A'); ?></p>
                    </div>
