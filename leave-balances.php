...
$employee_balances = [];
if (($employee_type_filter === 'all' || $employee_type_filter === 'employee') && ($current_tab === 'employee' || $current_tab === 'all')) {
    try {
        $employee_where_conditions = ["flb.year = ?"];
        $employee_params = [$year_filter];
        $employee_types = "i";
        ...
        $employee_query = "SELECT flb.*, 
                         e.first_name, e.last_name, f.id as employee_id, e.department, 'employee' as employee_type,
                         lt.name as leave_type_name, lt.description, lt.default_days_per_year,
                         'employee' as source_table
                         FROM employee_leave_balances flb
                         JOIN employees f ON flb.employee_id = f.id
...
$all_balances = array_merge($employee_balances, $employee_balances);
...
$all_balances_for_stats = array_merge($employee_balances, $employee_balances);
...
$total_employee = count($unique_employee);

// Calculate current tab statistics
$current_tab_balances = $current_tab === 'employees' ? $employee_balances : ($current_tab === 'employee' ? $employee_balances : $all_balances);
...
<div class="mb-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">View and manage employee and employee leave balances</p>
        </div>
...
<div class="mb-6">
    <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            <a href="?tab=employee<?php echo $year_filter ? '&year=' . $year_filter : ''; ?><?php echo $department_filter ? '&department=' . urlencode($department_filter) : ''; ?><?php echo $search ? '&search=' . urlencode($search) : ''; ?>" 
               class="<?php echo (isset($_GET['tab']) && $_GET['tab'] === 'employee') ? 'border-seait-orange text-seait-orange' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'; ?> whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm">
                <i class="fas fa-chalkboard-teacher mr-2"></i>Employee
                <span class="ml-2 bg-gray-100 text-gray-900 py-0.5 px-2.5 rounded-full text-xs font-medium"><?php echo $total_employee; ?></span>
            </a>
        </nav>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-lg p-6 mb-6 border border-gray-200">
    <form method="GET" id="leave-balance-filters" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="<?php echo ($current_tab === 'employees' || $current_tab === 'employee') ? 'hidden' : ''; ?>">
            ...
            <select name="employee_type" class="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-seait-orange focus:ring-2 focus:ring-seait-orange/20 transition-all">
                ...
                <option value="employee" <?php echo $employee_type_filter === 'employee' ? 'selected' : ''; ?>>Employee</option>
            </select>
        </div>
        <div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-gray-900"><?php echo $total_employee; ?></p>
            </div>
        </div>
    </div>
