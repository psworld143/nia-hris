<?php
/**
 * Enhanced Leave Allowance Calculator v2.0
 * Uses separate tables for Employee and Faculty leave allowance computation
 * 
...
class LeaveAllowanceCalculatorV2 {
    private $conn;
    ...
    public function calculateLeaveAllowance($employee_id, $employee_type, $year, $leave_type_id = 1) {
        try {
            ...
            // Double-check: Non-regular employees/faculty should never have accumulated leave
            if (!$is_regular) {
                $accumulated_days = 0;
                $can_accumulate = false;
                $accumulation_start_year = null;
    ...
    /**
     * Get year balance from separate tables (employee_leave_allowances or faculty_leave_allowances)
     */
    private function getYearBalance($employee_id, $employee_type, $year, $leave_type_id) {
        if ($employee_type === 'employee') {
            $query = "SELECT * FROM employee_leave_allowances 
                     WHERE employee_id = ? AND year = ? AND leave_type_id = ?";
        } else {
            $query = "SELECT * FROM faculty_leave_allowances 
                     WHERE faculty_id = ? AND year = ? AND leave_type_id = ?";
        }
        
        $stmt = mysqli_prepare($this->conn, $query);
        mysqli_stmt_bind_param($stmt, 'iii', $employee_id, $year, $leave_type_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    ...
    /**
     * Update or create leave balance record in separate tables
     */
    public function updateLeaveBalance($employee_id, $employee_type, $year, $leave_type_id) {
        ...
        // Use appropriate table based on employee type
        if ($employee_type === 'employee') {
            $table = 'employee_leave_allowances';
            $id_field = 'employee_id';
        } else {
            $table = 'faculty_leave_allowances';
            $id_field = 'faculty_id';
        }
        
        // Check if record exists
        $check_query = "SELECT id FROM $table 
                       WHERE $id_field = ? AND year = ? AND leave_type_id = ?";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            mysqli_stmt_bind_param($stmt, 'iiiiiiiisiii', 
                ...
                $balance_data['regularization_date'], $employee_id, $year, $leave_type_id);
        } else {
            // Insert new record
            $insert_query = "INSERT INTO $table 
                            ($id_field, leave_type_id, year, base_days, accumulated_days, 
                             total_days, used_days, remaining_days, is_regular, can_accumulate, 
                             accumulation_start_year, regularization_date) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            $stmt = mysqli_prepare($this->conn, $insert_query);
            mysqli_stmt_bind_param($stmt, 'iiiiiiiiiiss', 
                $employee_id, $leave_type_id, $year, $balance_data['base_days'],
    ...
    public function initializeYearlyLeaveBalances($year) {
        ...
        $leave_types_query = "SELECT id FROM leave_types WHERE is_active = 1";
        $leave_types_result = mysqli_query($this->conn, $leave_types_query);
        $leave_types = [];
        ...
        while ($row = mysqli_fetch_assoc($employee_result)) {
            // Initialize all leave types for each employee
            foreach ($leave_types as $leave_type_id) {
                if ($this->updateLeaveBalance($row['id'], 'employee', $year, $leave_type_id)) {
                    $success_count++;
                } else {
                    $error_count++;
                }
        ...
        while ($row = mysqli_fetch_assoc($faculty_result)) {
            // Initialize all leave types for each faculty
            foreach ($leave_types as $leave_type_id) {
                if ($this->updateLeaveBalance($row['id'], 'faculty', $year, $leave_type_id)) {
                    $success_count++;
                } else {
                    $error_count++;
                }
    ...
    /**
     * Get leave balance summary for HR dashboard using separate tables
     */
    public function getLeaveBalanceSummary($year = null) {
        ...
        $stmt = mysqli_prepare($this->conn, $employee_query);
        mysqli_stmt_bind_param($stmt, 'i', $year);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $summary['employee'] = mysqli_fetch_assoc($result);
        
        // Get faculty summary
        $faculty_query = "SELECT 
                            'faculty' as employee_type,
                            COUNT(DISTINCT faculty_id) as total_employees,
                            SUM(base_days) as total_base_days,
                            SUM(accumulated_days) as total_accumulated_days,
                            SUM(total_days) as total_available_days,
                            SUM(used_days) as total_used_days,
                            ...
                            SUM(CASE WHEN is_regular = 1 THEN 1 ELSE 0 END) as regular_employees,
                            SUM(CASE WHEN can_accumulate = 1 THEN 1 ELSE 0 END) as can_accumulate_count
                          FROM faculty_leave_allowances 
                          WHERE year = ?";
        
        $stmt = mysqli_prepare($this->conn, $faculty_query);
        mysqli_stmt_bind_param($stmt, 'i', $year);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $summary['faculty'] = mysqli_fetch_assoc($result);
