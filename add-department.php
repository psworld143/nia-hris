...
$icon = isset($_POST['icon']) ? trim($_POST['icon']) : 'fas fa-building';
...
if (empty($code)) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Department code is required']);
    exit();
}
...
$insert_query = "INSERT INTO departments (name, code, description, icon, color_theme, sort_order, is_active, created_by) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

$insert_stmt = mysqli_prepare($conn, $insert_query);
mysqli_stmt_bind_param($insert_stmt, 'sssssiii', 
    $name, 
    $code,
    $description,
    $icon,
